...
var BrowserSearchEngines = Class({
  ...
  // only the URL param is required
  addUrl : function add(url, options) {
    options = merge({ dataType : 1, iconURL : "", confirm : false }, options || {});
    SearchService.addEngine(url, options.dataType, options.iconURL, options.confirm);
  },
